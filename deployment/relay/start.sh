#!/bin/bash
# Start Relay Node Node

set -e

echo "🚀 Starting AnonNet Relay Node"
echo "================================="
echo ""
...
if [ -f data/api_port.txt ]; then
    ...
    echo ""
    echo "💡 Monitor: tail -f relay.log"
    echo "🛑 Stop: ./stop.sh"
else
    echo "⚠️ Daemon started but API not ready. Check relay.log"
fi
